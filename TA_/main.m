datanama = dir(strcat('D:\Nuzul Kuliah\TA\dataset\*.jpg'));
for i=1:length(datanama)    
    %% Load Data
    tmp = strcat('D:\Nuzul Kuliah\TA\dataset\',datanama(i).name);
    fileo = strcat('D:\Nuzul Kuliah\TA\newMethod\', datanama(i).name);
    img = imread(tmp);
    
    %% ANISOTROPIC FILTER TO HAVE A BETTER EDGE DETECTION

    %Anisotropic diffusion filter to preserve edges
    im=anisodiff2D(img, 10, 1/7, 20, 1);
